...
(function() {
  ...
  function waitForJsonData(callback) {
    if (typeof JsonData !== 'undefined' && JsonData.load) {
      ...
      setTimeout(function() { waitForJsonData(callback); }, 100);
    }
  }
        ...
        .then(function(data) {
          ...
          for (let i = 0; i < chiusure.length; i++) {
            ...
            if (chiusura.tipo === "periodo" && chiusura.inizio && chiusura.fine) {
              ...
              console.log(`📌 Verifico: ${chiusura.inizio}-${chiusura.fine} (${motivo})`);
              console.log(`   Oggi: ${oggiTimestamp}, Inizio: ${inizioTimestamp}, Fine: ${fineTimestamp}`);
          ...
          // Mostra il banner
          if (activeClosure) {
            const motivoText = (activeClosure.motivo !== "Ferie") ? ` - ${activeClosure.motivo}` : "";
            ...
            const tipoChiusura = (upcomingClosure.motivo === "Ferie") ? "ferie" : upcomingClosure.motivo;
