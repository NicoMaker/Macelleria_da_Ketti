...
function createFooterHTML(data, giornoPartenza) {
      ...
      if (oraCorrente >= inizioTime && oraCorrente < fineTime) {
        if (oraCorrente >= inChiusuraTime) {
          ...
          const minutiMancanti =
            oreFine * 60 + minFine - (oreCorr * 60 + minCorr);
  ...
  const orariHtml = giorniDaVisualizzare
    .map((dataDelGiorno, i) => {
      ...
      let testoOrario;
      const orariExtra = getOrariExtraForDate(
        data,
        dataFormattata,
        dayOfWeek
      );
      ...
      if (orariExtra) {
        ...
        if (closureCheck?.reason === "festivita")
          testoOrario = `${nomeGiorno}: Chiuso (Festività)`;
        if (closureCheck?.reason === "ferie")
          testoOrario = `${nomeGiorno}: Chiuso (Ferie fino al ${closureCheck.dataChiusura})`;
        if (closureCheck?.reason === "motivi-extra")
          testoOrario = `${nomeGiorno}: Chiuso (${closureCheck.motivoSpecifico})`;
      }
      ...
      if (i === 0) {
        ...
        if (eChiusoOggi || statoApertura.stato === "chiuso")
          ...
          colore = legenda.colori["in chiusura"] || "#FFD700";
        else colore = legenda.colori.aperto || "#00FF7F";
      } else {
        colore = legenda.colori.chiuso || "orange";
      }
  ...
  const testoInChiusuraSpan =
    statoApertura.stato === "in-chiusura"
      ? `In chiusura tra ${statoApertura.minutiAllaChiusura} minuti`
      : legenda.testo["in chiusura"] || "In chiusura";
  ...
  <div class="footer-content">
    <div class="footer-grid">
      ...
      <div class="footer-section">
        <h4 class="footer-subtitle">Contatti</h4>
        <ul class="footer-list">
          ${contatti.telefono ? `<li><a href="tel:${contatti.telefono}">${contatti.telefono}</a></li>` : ""}
          ${contatti.email ? `<li><a href="mailto:${contatti.email}">${contatti.email}</a></li>` : ""}
          ${contatti.indirizzo ? `<li>${contatti.indirizzo}</li>` : ""}
        </ul>
      </div>
      ...
      <div class="footer-section">
        ...
        <div class="social-links">
          ${social.facebook ? `<a href="${social.facebook}" target="_blank">FB</a>` : ""}
          ${social.instagram ? `<a href="${social.instagram}" target="_blank">IG</a>` : ""}
          ${social.whatsapp ? `<a href="${social.whatsapp}" target="_blank">WA</a>` : ""}
        </div>

        <div class="legenda-orari" style="margin-top:16px;">
          <h4 class="footer-subtitle">${legenda.titolo || "Legenda"}</h4>

          <div><span style="background:${legenda.colori.aperto};"></span>${legenda.testo.aperto}</div>
          <div><span style="background:${legenda.colori["in chiusura"]};"></span><span id="testo-in-chiusura">${testoInChiusuraSpan}</span></div>
          <div><span style="background:${legenda.colori.chiuso};"></span>${legenda.testo.chiuso}</div>
        </div>
      </div>
